  ...
  return (
    <div className=' max-w-[1520px] m-auto px-4 py-12'>
        ...
        <div className='grid md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-4 gap-6 py-4'>
            {
                foods.map((item)=>{
                 return   <div key={item.id} className='border-none hover:scale-105 duration-300'>
                        <img src={item.image} alt="" className='w-full h-[200px] object-cover rounded-lg'/>
                        <div className=' flex justify-between py-2 px-2 text-center'>
                        <p className=' font-bold'>{item.name}</p>
                            <p className=' bg-orange-700 h-16 w-16 rounded-full -mt-10 text-white border-8 grid place-items-center font-bold'>{item.price}</p>
                        </div>
